...
namespace zf {
  ...
  void ZFinderEvent::InitReco(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    found_prompt_jpsi_from_electrons = false;
    for (unsigned int i = 0; i < reco_jpsi_from_electrons.m.size() ; ++i ) {
      if (reco_jpsi_from_electrons.has_muons_in_eta_window.at(i) && reco_jpsi_from_electrons.is_in_rap_window.at(i) ) {
        found_dimuon_jpsi_from_electrons_with_muons_in_eta_window = true;
        //TODO should each cut level have its own flag??
        if ( reco_jpsi_from_electrons.has_high_pt_muons.at(i) && reco_jpsi_from_electrons.is_high_pt.at(i) ) {
          //TODO rename this flag or use an additional flag for high pT j/psi
          found_dimuon_jpsi_from_electrons_with_high_pt_muons = true;
            ...
            if (reco_jpsi_from_electrons.has_muons_with_compatible_vertex.at(i) ) {
              ...
              if (reco_jpsi_from_electrons.has_dimuon_vertex_compatible_with_primary_vertex.at(i)) {
                ...
                if (reco_jpsi_from_electrons.is_within_jpsi_mass_window.at(i) ) {
                  ...
                  if (reco_jpsi_from_electrons.is_prompt.at(i) ) {
                    found_prompt_jpsi_from_electrons = true;
                  }
                }
              }
            }
  ...
  void ZFinderEvent::InitJPsiFromElectrons(const reco::GsfElectron &e0, const reco::GsfElectron &e1, const TransientVertex &dielectron_vertex) {
    ...
    //math::PtEtaPhiMLorentzVector e0lv_boosted(e0.pt(), e0.eta(), e0.phi(), MUON_MASS);
    //e0lv_boosted.boost(jpsi_lv.px() / jpsi_lv.energy(), jpsi_lv.py() / jpsi_lv.energy(), jpsi_lv.pz() / jpsi_lv.energy());
    //math::BetaVector beta_vector;
    ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<double> >::BetaVector beta_vector;
    beta_vector = jpsi_lv.BoostToCM();
    //std::cout << "px " << jpsi_lv.px() << " bv_x  " << beta_vector.x() << std::endl;
    ...
    //std::cout << "jpsi px " << jpsi_lv.px() << " e0 px " << e0lv.px() << " boosted px " << e0_lv2->Px() << std::endl;
    ...
    //}
    //else {
    //  reco_jpsi_from_electrons.muon0.push_back (e1);
    //  reco_jpsi_from_electrons.muon1.push_back (e0);
